...
#define INSTANCE_RUNNING_ERROR_CODE 5 // my return code when there was a running instance ;)
...
public:
    ...
    void crashHandler()
    {
        ...
        signal(SIGHUP, SingleApplicationPrivate::terminate); // added by me, terminate app without a terminal
    }
 ...
 * @param argv
 */
SingleApplication::SingleApplication(int &argc, char **argv, const QString &instanceRunningMessage)
    : QAPPLICATION_CLASS(argc, argv), d_ptr(new SingleApplicationPrivate(this)), m_instanceRunningMessage(instanceRunningMessage)
{
    ...
    if( d_ptr->memory->create(1, QSharedMemory::ReadOnly) )
    {
        ...
        {
            this->printInstanceRunningMessage();
            ::exit(INSTANCE_RUNNING_ERROR_CODE); // Terminate the program using STDLib's exit function
        } else {
            this->printInstanceRunningMessage();
            delete d_ptr->memory;
            ::exit(INSTANCE_RUNNING_ERROR_CODE);
        }
    }
}
...
void SingleApplication::printInstanceRunningMessage() const
{
    std::cout << qUtf8Printable(this->m_instanceRunningMessage) << std::endl;
}
